...
@section('heading','Create Final Actual Budget : Your actual budget must not exceed K'. $departmentIncome.'.00
as allocated to your department by the School')
            ...
            <div class="panel panel-default">
                <div class="panel-heading ">
                    @if(isset($dpName)) <div class="text-primary">The Department Of {{ $dpName }} |
                        Now you are remaining with k {{ $totalBudget }}.00 to complete making your budget</div> @endif
                </div>
                        ...
                        @if(isset($records))
                            @foreach( $records as $rcd)
                                ...
                                <tr>
                                    ...
                                    <td> @if(isset($rcd))
                                        <div class="btn-group">
                                            <a href="{{ route('/modify', ['id' => $rcd->id]) }}"
                                               class="btn btn-sm btn-link"><i class="fa fa-info-circle fa-fw text-success">
                                                </i><span class="text-primary">Modify</span></a></div>

                                        <div class="btn-group">
                                            <a href="{{ route('/saveAsFinal', ['id' => $rcd->id]) }}" class="btn btn-sm btn-link"><i class="fa fa-info-circle fa-fw text-success">
                                                </i><span class="text-success">Save as final</span></a></div>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
